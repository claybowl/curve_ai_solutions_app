...
CREATE POLICY "Public categories" ON assessment_categories FOR SELECT USING (true);
CREATE POLICY "Public questions" ON assessment_questions FOR SELECT USING (is_active = true);
...
CREATE POLICY "Consultants view assigned consultations" ON consultations
    FOR SELECT USING (auth.uid() = assigned_consultant_id);

-- Tools and prompts policies
CREATE POLICY "Public tool categories" ON tool_categories FOR SELECT USING (true);
CREATE POLICY "Public tools" ON ai_tools FOR SELECT USING (is_public = true);
CREATE POLICY "Public prompt categories" ON prompt_categories FOR SELECT USING (true);
CREATE POLICY "Public prompts" ON prompts FOR SELECT USING (is_public = true);

-- User-specific data policies
CREATE POLICY "Users manage own tool usage" ON user_tool_usage 
    FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Users manage own prompt collections" ON user_prompt_collections 
    FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Users manage own collection items" ON prompt_collection_items 
    FOR ALL USING (
        EXISTS (
...
-- Admin policies
CREATE POLICY "Admins full access" ON assessment_categories 
    FOR ALL USING (
        EXISTS (SELECT 1 FROM profiles WHERE user_id = auth.uid() AND role = 'admin')
    );

CREATE POLICY "Admins manage questions" ON assessment_questions 
    FOR ALL USING (
        EXISTS (SELECT 1 FROM profiles WHERE user_id = auth.uid() AND role = 'admin')
    );
...
CREATE POLICY "Admins manage tools" ON ai_tools 
    FOR ALL USING (
        EXISTS (SELECT 1 FROM profiles WHERE user_id = auth.uid() AND role = 'admin')
    );
